...
async function fetchCarModelsWithGeneration(): Promise<CarModelWithGeneration[]> {
  console.log('🚀 Rozpoczynam pobieranie rekordów z polem generation...\n');

  const allRecords: CarModelWithGeneration[] = [];
  const pageSize = 150;
  let offset = 0;
  let hasMore = true;
  let totalFetched = 0;
  ...
  try {
    ...
    while (hasMore) {
      ...
      const { data, error } = await supabase
        .from('car_models_extended')
        .select('id, brand_name, model_name, generation')
    ...
    const recordsWithGeneration = allRecords.filter(r => r.generation && r.generation.trim() !== '');
    console.log(`📅 Rekordy z polem generation: ${recordsWithGeneration.length} (${((recordsWithGeneration.length / allRecords.length) * 100).toFixed(1)}%)`);

    // Pokaż przykłady generation
    console.log('\n📝 Przykłady generation:');
    recordsWithGeneration.slice(0, 10).forEach(record => {
      console.log(`   • ${record.brand_name} ${record.model_name}: "${record.generation}"`);
    });
    ...
    const dataPath = path.join(outputDir, 'car-models-with-generation.json');
    fs.writeFileSync(dataPath, JSON.stringify(allRecords, null, 2));
    console.log(`\n💾 Zapisano dane do: ${dataPath}`);
...
export { fetchCarModelsWithGeneration, type CarModelWithGeneration };
