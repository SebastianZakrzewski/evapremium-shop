        ...
        <div className={`lg:hidden fixed top-16 md:top-20 left-0 w-full bg-black/95 backdrop-blur-xl border-b border-white/10 z-[40] shadow-2xl pb-safe transition-all duration-300 ease-in-out overflow-hidden ${open ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="flex flex-col gap-4 py-8 px-6">
            ...
            <div className="mt-6 pt-6 border-t border-white/10 space-y-3">
              <a href="tel:+48793993430" className="flex items-center justify-center gap-3 text-white font-bold text-lg bg-white/5 p-4 rounded-xl border border-white/10" onClick={() => setOpen(false)}>
              ...
              <button
                type="button"
                onClick={handleOpenChat}
                className="w-full flex items-center justify-center gap-3 text-white font-medium text-base bg-white/5 p-4 rounded-xl border border-white/10 hover:bg-white/10 transition-colors"
              >
                <svg className="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                Chat z asystentem 24/7
              </button>
            </div>
          </div>
        </div>
