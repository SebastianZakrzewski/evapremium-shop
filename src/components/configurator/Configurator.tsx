...
export default function Configurator() {
  ...
  // Mapowanie nazw marek z carousel na nazwy w bazie danych - używa shared utilities
  const getBrandNameForAPI = (brandParam: string | null) => {
    if (!brandParam) return "";
    
    const normalized = normalizeBrandName(brandParam.toLowerCase().trim());
    if (normalized) {
      console.log(`🔍 Configurator: Mapped brand "${brandParam}" -> "${normalized}"`);
      return normalized;
    }
