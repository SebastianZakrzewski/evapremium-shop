...
export function useMat(params: UseMatParams): UseMatReturn {
  ...
  const fetchMat = async () => {
    ...
    try {
      const foundMat = await matsApi.findMat({
      ...
      if (foundMat) {
        debugLog('useMat: Mat found', foundMat);
        setMat(foundMat);
      } else {
...
export function useAvailableBodyTypes(brandSlug?: string, modelSlug?: string) {
  ...
  useEffect(() => {
    ...
    const fetchBodyTypes = async () => {
      ...
      try {
        const bodyTypesData = await matsApi.getBodyTypes(brandSlug, modelSlug);
        debugLog('useAvailableBodyTypes: Found body types', bodyTypesData);
